...
genre_rev <- movie %>% group_by(Genre, runtime_cat) %>% 
  summarise(Revenue = mean(Adjusted_Gross), Avg_budget = mean(Budget),
            ...
            Avg_Profit_Perc = mean(Profit_perc)) 


#Plot a bar graph by Genre and runtime bucket
genre_rev = genre_rev[order(genre_rev$Avg_budget),]
ggplot(data=genre_rev,aes(y=Avg_budget,x=Genre))+
...
ggplot(data=genre_rev,aes(y=Avg_Overseas_Profit,x=Genre))+
  ...
  coord_flip()+
  facet_grid(runtime_cat~.,)
ggplot(data=genre_rev,aes(y=Avg_Profit_Perc,x=Genre))+
  geom_bar(stat = "identity", fill = 'orange', width = 0.2)+
  ylab("Profit ($Bn)")+
  xlab("Genre")+
  ggtitle("Average Profit Percentage by Genre")+
  coord_flip()+
  facet_grid(runtime_cat~.,)
